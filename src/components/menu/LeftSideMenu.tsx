...
const AvailableMenuItem: React.FC<PropsWithChildren<{ item: MenuItem }>> = ({ item }) => {
...
export default function LeftSideMenu() {
  return (
    <>
      ...
      {MENU_ITEMS.map((item) => (item.available ? <AvailableMenuItem key={item.id} item={item} /> : <NotAvailableMenuItem key={item.id} item={item} />))}
    </>
  );
}
