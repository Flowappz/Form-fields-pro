    ...
    return (
        <div className="pb-2 m-0 flex flex-col gap-1">
            <div className='flex items-center gap-2'>
                {label ? <span
                    className="text-[0.77rem] box-border inline-block text-[#ABABAB] m-0 p-0">{label}</span> : null}
                <div
                    className='w-4 h-4 border border-white rounded-full cursor-pointer' style={{backgroundColor: `${value}`}}
                    onClick={() => setShowPicker(prev => !prev)}
                ></div>
            </div>
            <div className="relative" tabIndex={-1} onFocus={() => setShowPicker(true)}
